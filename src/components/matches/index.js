...
const EmptyTextRow = (props) => {
  ...
  return (
    <div
      className="btn btn-disabled match-detail col"
      style={{
        width: "100%",
        height: "30px",
        padding: "2px",
...
const SideBets = (props) => {
  ...
  return (
    <div
      className={`bet-fix col-lg-1 col-sm-1 col-md-1 col-xs-1 events-odd pad ${picked}`}
    >
      <a
        className="side"
...
const OddButton = (props) => {
  ...
  const handleButtonOnClick = (event) => {
    ...
    if (cstm === ucn) {
      ...
      if (picked === "picked") {
        betslip =
          jackpot !== true ? removeFromSlip(mid) : removeFromJackpotSlip(mid);

        setPicked("");
      } else {
        betslip = jackpot !== true ? addToSlip(slip) : addToJackpotSlip(slip);
  ...
  return (
    ...
    >
      ...
      {detail && (
        <>
          <span className="label label-inverse blueish">{match.odd_key}</span>
    ...
    <div className="matches full-width mt-5">
      ...
      <Container className="web-element">
        ...
        {matches !== null && matches.length === 0 && (
          <div className="top-matches row">No events found.</div>
        )}
      </Container>
    </div>
  );
